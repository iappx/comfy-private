...
Двойник собирается за минуту против получаса на реальный образ, а ломается в CI ровно тот
...
с образом — правка `entrypoint.sh` пересобирает образ за пару минут вместо получаса.
...
Размер `/dev/shm` печатается в лог при старте. Docker по умолчанию даёт 64 МБ; если у пода
окажется столько, места хватит на десяток картинок, и придётся забирать результаты по SFTP
почаще либо смириться с записью вывода на container disk.
